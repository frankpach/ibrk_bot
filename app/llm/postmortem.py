...
Llama al LLM para extraer un patrón aprendido y lo guarda en DB (upsert para evitar duplicados).
"""
import logging
...
from app.db.database import upsert_pattern
...
def run_postmortem(trade: Trade):
    """Analiza un trade cerrado y extrae/actualiza un patrón aprendido."""
    if not LLM_API_KEY:
    ...
    try:
        ...
        logger.info(f"Postmortem pattern saved/updated for {trade.symbol}: {pattern_text}")
