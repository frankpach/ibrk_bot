    ...
    for signal in signals:
        ...
        try:
            ...
            if decision.action in ("BUY", "SELL"):
                ...
                order_placed = True  # Se procesó correctamente aunque no haya orden

        except Exception as e:
            logger.error(f"Error processing signal {signal.id} for {signal.symbol}: {e}")
            # No marcamos como procesada para reintentar en el próximo ciclo
            continue
...
def _execute_order(symbol: str, decision: LLMDecision) -> bool:
    """Envía la orden a FastAPI — retorna True si la orden fue aceptada."""
    payload = {
        "symbol": symbol,
